...
class Qwen2Attention(nn.Module):
    ...
    def __init__(self, config: Qwen2Config, layer_idx: int, adapt=False):
        super().__init__()
        self.config = config
        ...
        self.adapt = adapt
        if self.adapt:
            self.adapter_q = nn.Sequential(
                nn.Linear(config.hidden_size,  self.lora_dim),
                nn.Linear(self.lora_dim, config.num_attention_heads * self.head_dim)
            )
            self.adapter_v = nn.Sequential(
                nn.Linear(config.hidden_size,  self.lora_dim),
                nn.Linear(self.lora_dim, config.num_key_value_heads * self.head_dim)
            )
    def forward(
        self,
        ...
        **kwargs: Unpack[FlashAttentionKwargs],
        ...
        if adapt and self.adapt:
            adapt_query_states = self.adapter_q(hidden_states).view(hidden_shape).transpose(1, 2)
            adapt_value_states = self.adapter_v(hidden_states).view(hidden_shape).transpose(1, 2)
            query_states += adapt_query_states
            value_states += adapt_value_states
...
class Qwen2DecoderLayer(nn.Module):
    def __init__(self, config: Qwen2Config, layer_idx: int):
        ...
        self.self_attn = Qwen2Attention(config=config, layer_idx=layer_idx, adapt=(layer_idx in list(range(self.start_conv_idx, self.end_conv_idx))))
