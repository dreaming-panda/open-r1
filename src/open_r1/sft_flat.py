...
def main(script_args, training_args, model_args):
    ...
    layers = copy.deepcopy(model.model.layers[:config.end_conv_idx])

    # Unroll the convolutions
    for _ in range(config.num_conv):
        layers.extend(copy.deepcopy(model.model.layers[config.start_conv_idx:config.end_conv_idx]))

    # Append remaining layers
    layers.extend(copy.deepcopy(model.model.layers[config.end_conv_idx:]))
...
if __name__ == "__main__":
    parser = TrlParser((ScriptArguments, SFTConfig, ConvModelConfig))
