    ...
    def forward(
        ...
        **flash_attn_kwargs: Unpack[FlashAttentionKwargs],
        ...
        for conv_id in range(self.num_conv):
            ...
            router_states = torch.cat([hidden_states, residual], dim=-1)
            if self.gradient_checkpointing and self.training:
                router_score: torch.Tensor = self._gradient_checkpointing_func(
                    self.router.__call__,
                    router_states,
                )
            else:
                router_score: torch.Tensor = self.router(
                    router_states,
                )
            

            for layer_offset in range(self.conv_block_size):
                
                layer_idx = self.start_conv_idx + layer_offset
                execution_idx = self.end_conv_idx + self.conv_block_size * conv_id + layer_offset
                decoder_layer = self.layers[layer_idx]
            ...
            router_score.masked_fill_(router_score < 0.5, 0.0)
            hidden_states = residual * (1 - router_score) + hidden_states * router_score
        
        for layer_idx in range(self.end_conv_idx, self.num_hidden_layers):
            
            execution_idx = layer_idx + self.conv_block_size * self.num_conv
            decoder_layer = self.layers[layer_idx]
            if output_hidden_states:
                all_hidden_states += (hidden_states,)
