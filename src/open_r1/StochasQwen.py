...
class Qwen2Model(Qwen2PreTrainedModel):
    ...
    def __init__(self, config: Qwen2Config):
        ...
        self.proba = torch.linspace(1, 0.5, steps=self.num_conv)
    ...
    def forward(
        ...
        **flash_attn_kwargs: Unpack[FlashAttentionKwargs],
        ...
        r = torch.rand(self.num_conv).to(self.device)
        for conv_id in range(self.num_conv):
            if r[conv_id] >= self.proba[conv_id]:
                continue
            for offset in range(self.conv_block_size):
                layer_idx = self.start_conv_idx + conv_id * self.conv_block_size + offset
                decoder_layer = self.layers[layer_idx]
                if self.gradient_checkpointing and self.training:
                    layer_outputs = self._gradient_checkpointing_func(
                        decoder_layer.__call__,
        ...
        for layer_idx in range(self.end_conv_idx, self.config.num_hidden_layers):
            decoder_layer = self.layers[layer_idx]
