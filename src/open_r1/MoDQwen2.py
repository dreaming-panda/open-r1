    ...
    def forward(
        ...
        **flash_attn_kwargs: Unpack[FlashAttentionKwargs],
        ...
        for conv_id in range(self.num_conv):
            ...
            router_states = torch.cat([hidden_states, conv_input], dim=-1)
            if self.gradient_checkpointing and self.training:
            ...
            for layer_offset in range(self.conv_block_size):
                ...
                execution_idx = self.start_conv_idx + self.conv_block_size * conv_id + layer_offset
                decoder_layer = self.layers[layer_idx]
        ...
        for layer_idx in range(self.end_conv_idx, self.num_hidden_layers):
            ...
            execution_idx = layer_idx + self.conv_block_size * (self.num_conv - 1)
            decoder_layer = self.layers[layer_idx]
            if output_hidden_states:
                all_hidden_states += (hidden_states,)
